...
Після дослідження продуктивності, видно що перший алгорит
...
### Формат виводу
Кожен розв'язок кросворду виводиться у вигляді сітки, де слова позначені символами, а вільні клітинки - пустими місцями або дефіфівцсами.
...
Прикладом використання backtracking у вирішенні задачі задоволення обмеень. Такою задачею є Magnetic Puzzle, для якої потрібно вже заданого поля вставити магнітики\пустишки. Сама ж реалізація backtracking з'являється при переборі комірок поля при вставленні магнітика\пустишки у заданій кількості для +\- по горизонталям\вертикалям. 

### Введення даних: 
Задається поле та обмеження на кількість +\- у вигляді масиву та списків.

### Формат виводу:
Приклад: [['+', '-', '+', '-', 'x', '-', ]['-', '+', '-', '+', 'x', '+', ]['x', 'x', '+', '-', '+', '-', ]['x', 'x', '-', '+', 'x', '+', ]['-', '+', 'x', 'x', 'x', '-', ]]
...
Backtracking - це метод розв'язання задачі, при якому систематично досліджуються всі можливі варіанти розв'язання шляхом "відкату" до попереднього кроку при досягненні тупика. Розглянемо розв'язання криптарифметичної головоломки (cryptarithmetic puzzle) з використанням методу backtracking.

Криптарифметика - це головоломка, де буквам замінено цифри, і потрібно знайти відповідність цифр і букв так, щоб математичне рівняння було вірним.

Приклад головоломки: SEND + MORE = MONEY

### Введення даних: 

Визначаємо всі унікальні букви в рівнянні (S, E, N, D, M, O, R, Y).
Встановлюємо правило, що кожній букві відповідає унікальна цифра від 0 до 9.
Backtracking:

Починаємо зі спроби встановити цифру для першої букви (S або M). Перебираємо всі можливі цифри від 0 до 9.
Якщо поточна комбінація цифр не порушує правила (наприклад, жодні дві букви не відповідають одній цифрі), переходимо до наступної букви.
Якщо всі букви отримали відповідні цифри і рівняння вірне, рішення знайдено.
Якщо на якому-небудь етапі виникає порушення правил (наприклад, дві букви отримали однакову цифру), повертаємося на крок назад (backtrack) і змінюємо комбінацію цифр.
Продовжуємо цей процес, поки не знайдемо правильну комбінацію цифр для всіх букв або поки не вичерпаємо всі можливі варіанти.
Приклад:

Для рівняння SEND + MORE = MONEY починаємо з призначення цифри для S (0..9), потім E (0..9), і так далі.
Якщо на якому-небудь етапі ми виявляємо, що дві букви отримали однакову цифру (наприклад, S = 1 і E = 1), ми повертаємося на попередній крок і спробуємо іншу цифру для S або E.
Оптимізації:

Можна додати різні оптимізації, щоб прискорити пошук рішення, наприклад, враховувати різні комбінації цифр, які можуть бути відкинуті заздалегідь через особливості рівняння.

### Формат виводу:
Словник літера-цифра
